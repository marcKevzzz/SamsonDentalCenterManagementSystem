...
document.addEventListener('DOMContentLoaded', async () => {
    const data = await AdminStore.loadData('inquiries', '/api/admin/data/inquiries');
...
function initializeWithData(data) {
    ...
    if (!ACTIVE_INQUIRY_ID && ALL_INQUIRIES.length > 0 && window.innerWidth >= 768) {
...
function getDisplayName(inq) {
    ...
    if (inq.patient && inq.patient.firstName) return `${inq.patient.firstName} ${inq.patient.lastName}`;
    if (inq.patient && inq.patient.fullName) return inq.patient.fullName;
    
    const guestName = `${inq.guestFirstName || ""} ${inq.guestLastName || ""}`.trim();
    if (guestName) return guestName;
    
    if (inq.patientName && inq.patientName !== "null null") return inq.patientName;
...
function renderInquiryList() {
    ...
    container.innerHTML = ALL_INQUIRIES.map(inq => {
        ...
        const initials = displayName.length > 0 ? displayName[0].toUpperCase() : "G";
        const isPending = inq.status === 'pending';
        const date = new Date(inq.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });

        return `
            <div class="inquiry-item p-4 hover:bg-slate-50 cursor-pointer transition-all border-l-4 ${isPending ? 'border-amber-400 bg-amber-50/30' : 'border-transparent'} ${ACTIVE_INQUIRY_ID === inq.id ? 'bg-slate-100 border-primary' : ''}"
                ...
                <div class="flex items-center gap-3 mb-1 pointer-events-none">
                    ${avatarUrl ? `<img src="${avatarUrl}" class="w-8 h-8 rounded-full object-cover border border-slate-200" />` : 
...
async function loadInquiry(id, name, subject, avatarUrl, isActive, element) {
    ...
    document.querySelector(selectors.chatEmpty).classList.add('opacity-0', 'pointer-events-none');
            ...
            if (msgDate !== lastDate) {
                html += `
                    <div class="flex items-center gap-4 my-6">
                        ...
                        <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-white px-3 py-1 rounded-full border border-slate-100 shadow-sm">${formatDateSeparator(msg.created_at)}</span>
                        <div class="flex-1 h-px bg-slate-200"></div>
                    </div>`;
                lastDate = msgDate;
            }
