...
function renderDynamicContent() {
    ...
    if (hoursList && data.hours && Array.isArray(data.hours)) {
        hoursList.innerHTML = data.hours.map((h, i) => `
...
function updateClinicStatus() {
    ...
    if (!data.status.isAutomated) {
        ...
        if (todayHours && !todayHours.closed) {
            ...
            const noonStart = todayHours.noonStart ? parseInt(todayHours.noonStart.replace(':', '')) : null;
            const noonEnd = todayHours.noonEnd ? parseInt(todayHours.noonEnd.replace(':', '')) : null;

            if (currentTime >= openTime && currentTime < closeTime) {
                // Check for noon break
                if (noonStart && noonEnd && currentTime >= noonStart && currentTime < noonEnd) {
                    isOpen = false;
                ...
                isOpen = false;
                statusText = "Closed Now";
                subText = currentTime < openTime ? `Opens at ${formatTime(todayHours.open)}` : `Closed for today`;
            }
        }
    }
...
function initContactsAnimations() {
    ...
    gsap.fromTo(".mission-card", 
        ...
        {
            autoAlpha: 1, y: 0, scale: 1, duration: 0.8, stagger: 0.1, ease: "back.out(1.7)",
    ...
    gsap.fromTo("#contactSection .reveal-up", 
        { autoAlpha: 0, y: 50 },
        {
            autoAlpha: 1, y: 0, duration: 1, stagger: 0.3, ease: "power4.out",
...
function initFormInteractions() {
    ...
    inputs.forEach(input => {
        input.addEventListener("focus", () => {
            gsap.to(input, { borderColor: "#1E40AF", backgroundColor: "#fff", duration: 0.3 });
        });
        input.addEventListener("blur", () => {
            if (!input.value) {
                gsap.to(input, { borderColor: "#e5e7eb", backgroundColor: "#f9fafb", duration: 0.3 });
            }
        });
    });
}
        ...
        if (data.ok) {
            ...
            gsap.to(contactForm, {
                ...
                onComplete: () => {
                    ...
                    gsap.fromTo([successMsg, chatContainer], { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.8, stagger: 0.2, ease: "back.out(1.7)" });
                    fetchChatMessages();
                    startChatPolling();
                }
            });
...
window.sendFollowup = async function() {
    ...
    try {
        const res = await fetch('/api/inquiry/message', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ inquiryId: window.activeInquiryId, message: msg, isFromStaff: false })
        });
        if (res.ok) {
            input.value = '';
